...
class AP1roGSDGeneralized(FanCI):
    r"""
            ...
            for pair_exc_order in range(len(holes)+1):
                ...
                for _holes in holes_comb:
                    for _parts in parts_comb:
                        ...
                        # For AP1roGSDGeneralized_sen-o wavefunction, EACH singles excitation considered with respect to the reference state should satisfy one condition.
                        ...
                        if _check_paired_singles(holes_ab, self.nocc_up):
                            ...
                            y[i] += (olp * permant_S)
                            continue
                        olp = olp * permanent(t_i[holes_ab,:][:, parts_ab])                       
                        y[i] += olp
