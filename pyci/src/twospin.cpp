...
void TwoSpinWfn::from_file(const char *filename) {
  ...
  do {
    if (!(file.read((char *)&ndet, sizeof(int_t)) && file.read((char *)&nbasis, sizeof(int_t)) &&
          file.read((char *)&nocc_up, sizeof(int_t)) && file.read((char *)&nocc_dn, sizeof(int_t))))
      break;
...
void TwoSpinWfn::to_file(const char *filename) const {
  ...
  bool success =
      file.write((char *)&ndet, sizeof(int_t)) && file.write((char *)&nbasis, sizeof(int_t)) &&
      file.write((char *)&nocc_up, sizeof(int_t)) && file.write((char *)&nocc_dn, sizeof(int_t)) &&
...
#ifdef PYCI_EXACT_HASH
  return (uint_t)(rank_colex(nbasis, nocc_up, det) * maxdet_dn +
                  rank_colex(nbasis, nocc_dn, &det[nword]));
#else
...
double TwoSpinWfn::compute_overlap(const double *coeffs, const TwoSpinWfn &wfn,
                                   const double *w_coeffs) const {
